    ...
    <div class="card-body">
        <form action="{{ route("admin.tickets.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                ...
                <input type="text" id="title" name="title" class="text-uppercase form-control" value="{{ old('title', isset($ticket) ? $ticket->title : '') }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group {{ $errors->has('author_name') ? 'has-error' : '' }}">
                ...
                <input type="text" id="author_name" name="author_name" placeholder="Nombre Completo" class="text-capitalize form-control" value="{{ old('author_name', isset($ticket) ? $ticket->author_name : '') }}">
            ...
            <div class="form-group >
                ...
                <select name="department_id" id="department" class="form-control select2">
                    ...
                    @foreach($departments as $id => $department)
                        <option
                            value="{{ $id }}" {{ (isset($ticket) && $ticket->department ? $ticket->department->id : old('$department_id')) == $id ? 'selected' : '' }}>{{$department}}</option>
                    @endforeach
                </select>
            </div>
